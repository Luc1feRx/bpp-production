...
const buildSyncXFieldCatalogue = (): FieldOption[] => {
  return [
    // ===== ORDER BASIC =====
...
const includesSearch = (haystack: string, needle: string) => {
...
const OrderExportTemplateDetail = ({
  orders,
}: {
  orders: OrderField[];
}) => {
  ...
  const exportedTimestamp = useMemo(() => {
  ...
  const buildCSV = () => {
    ...
    const rows = orderList.map((order) => {
      return columns.map((c) => {
        if (c.path === "__static.exportedTimestamp") return exportedTimestamp;
  ...
  const downloadCSV = () => {
    ...
    const datePart = exportedTimestamp.slice(0, 10);
...
export default OrderExportTemplateDetail